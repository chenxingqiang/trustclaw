...
import {
  buildTraDataConsentDescription,
  createTrustclawTraDataConsentHook,
...
describe("trustclaw-tra data consent hook", () => {
  it("blocks trustclaw_tra_query when domain agent is not granted", async () => {
    const dir = mkdtempSync(path.join(tmpdir(), "trustclaw-consent-no-grant-"));
    const dbPath = path.join(dir, "local_tra.db");
    const auditDir = path.join(dir, "audit");
    try {
      initializeTra(sampleInitPayload, dbPath);
      const hook = createTrustclawTraDataConsentHook({ dbPath, auditDir });
      const result = await hook(
        { toolName: "trustclaw_tra_query", params: { message: "test" } },
  ...
  it("blocks trustclaw_tra_query when TRA is not mounted", async () => {
  ...
  it("blocks trustclaw_tra_write when TRA is not mounted", async () => {
    ...
    try {
      const hook = createTrustclawTraDataConsentHook({ dbPath, auditDir });
      const result = await hook(
        { toolName: "trustclaw_tra_write", params: { message: "record weight 72kg" } },
        { sessionKey: "sess_3" },
      );
      expect(result?.block).toBe(true);
    } finally {
  ...
  it("builds bounded consent descriptions", () => {
    const description = buildTraDataConsentDescription("x".repeat(200), [
      "患者姓名",
      "糖化血红蛋白 HbA1c",
    ]);
