...
import { createPtdsAuditEventsHandler } from "./audit-routes.js";
...
describe("audit routes agent scoping", () => {
  it("filters compliance and chat events by agentPackId", async () => {
    ...
    const handler = createPtdsAuditEventsHandler({ auditDir, dbPath });
    const { res, getBody } = mockRes();
    await handler(
      { method: "GET", url: `/api/ptds/audit/events?scope=all&agentPackId=${glp1.id}` } as never,
      res,
    );
