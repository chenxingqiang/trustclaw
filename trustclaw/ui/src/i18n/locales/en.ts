import type { TrustclawMessages } from "../types.js";

export const en: TrustclawMessages = {
  console: {
    title: "TrustClaw TRA Console",
    badge: "TrustClaw V1",
    systemStatus: "System status",
    statusRunning: "Running",
    statusNotReady: "Not ready",
    statusNotMounted: "TRA not mounted",
    statusChecking: "Checking…",
    statusReset: "Reset complete",
    toggleTheme: "Toggle theme",
    language: "Language",
    langEn: "English",
    langZh: "中文",
    chatFrameTitle: "TrustClaw Chat",
    chatInControlUiTitle: "TrustClaw Chat",
    chatInControlUiBody:
      "Embedded from Control UI. Prefer the TrustClaw TRA Console tab in gateway Control UI for the native chat workbench with collapsible side rails.",
    overviewAria: "TRA audit charter and dual-plane map",
    auditCharterTitle: "Fail-closed audit contract",
    auditCharterLead:
      "Every trust-runtime access, rule evaluation, and agent decision must leave a verifiable trail. Missing steps, failed read-only checks, or absent consent block the pipeline.",
    auditRuleFailClosed:
      "Fail-closed: BLOCKED or FAIL status stops downstream stages; no silent fallback.",
    auditRuleReadOnly:
      "Text2SQL: `read_only_verification` must be true; non-read-only SQL is a security breach.",
    auditRuleNoPhiDump:
      "Audit logs store SQL summaries and row counts only — never full PHI result sets.",
    auditRuleLedger:
      "Each successful chat run ends with EvidenceLedger.Commit (SHA-256 linked chain in Panel E).",
    auditRuleConsent:
      "DATA_CONSENT / COMPLIANCE_IMPORT / REFERENCE_SYNC / DEVICE_IMPORT are separate JSONL trails — required before imports.",
    dataPlaneTitle: "Data plane",
    dataPlaneDesc: "Local SQLite trust-runtime mount, browse, and governed external sync.",
    dataPlaneStepA: "Mount personal vitals & clinical flags into local trust runtime",
    dataPlaneStepC: "Grant per-domain-agent scopes — audited AGENT_DOMAIN_GRANT",
    dataPlaneStepB: "Inspect canonical tables (read-only viewer)",
    dataPlaneStepF: "Subscribe compliance / NRDL / device data with explicit consent",
    auditPlaneTitle: "Audit plane",
    auditPlaneDesc: "Immutable runtime audit + cryptographic evidence ledger.",
    auditPlaneStepD: "JSONL pipeline stages (compliance + latest chat trail)",
    auditPlaneStepE: "Hash-chained receipts; chain verify badge",
    chatWorkbenchHint:
      "Audited Chat runs in Control UI → TrustClaw TRA Console tab (center workbench). Agent tool calls feed Panel D/E here in real time.",
    columnDataPlane: "Data plane · local trust runtime",
    columnAuditPlane: "Audit plane · JSONL + ledger",
  },
  panels: {
    landing: {
      title: "A · TRA initialization",
      subtitle: "Step 0 — mount governed personal data before any audited query",
      notMounted: "Not mounted",
      mounted: "Mounted",
      sectionBasic: "Basic vitals",
      sectionHighRisk: "High-risk clinical status",
      sectionComorbid: "Comorbid conditions",
      sectionMedHistory: "Prior oral / insulin control history",
      sectionPrescription: "Prescription context (AST)",
      patientName: "Patient name",
      gender: "Gender",
      genderMale: "Male",
      genderFemale: "Female",
      age: "Age (years)",
      weight: "Weight (kg)",
      height: "Height (cm)",
      bmi: "BMI (auto-calculated)",
      hba1c: "HbA1c (%)",
      isPregnantOrLactating: "Pregnant or lactating",
      hasType2Diabetes: "Type 2 diabetes diagnosis",
      thyroidHistory: "Medullary thyroid cancer history",
      pancreatitisHistory: "Acute pancreatitis history",
      cardiovascularRisk: "Cardiovascular high-risk",
      gastrointestinalSensitivity: "Severe GI sensitivity",
      hasArteriosclerosis: "Arteriosclerosis",
      hasCoronaryHeartDisease: "Coronary heart disease",
      hasMyocardialInfarction: "Myocardial infarction",
      hasStroke: "Stroke",
      usedMetforminBadControl: "Metformin inadequate control",
      usedSulfonylureaBadControl: "Sulfonylurea inadequate control",
      usedInsulinBadControl: "Insulin inadequate control",
      isFirstPrescription: "First prescription for this therapy",
      isSpecialistPhysician: "Prescribing specialist physician",
      institutionLevel: "Institution level",
      institutionLevel1: "Level 1 (primary)",
      institutionLevel2: "Level 2 (secondary)",
      institutionLevel3: "Level 3 (tertiary)",
      initBtn: "Initialize and load data space",
      resetBtn: "Reset TRA data",
      mounting: "Mounting…",
      resetting: "Resetting…",
      initFailed: "Initialization failed",
    },
    agentGrants: {
      title: "C · Domain agent authorization",
      subtitle: "Per-agent scope grants with append-only AGENT_DOMAIN_GRANT audit history",
      description:
        "Grant each Business Agent its own TRA scopes. Panels B/D/E/F and chat tools fail closed until granted (runtime decoupled from agents).",
      domain: "Domain",
      loading: "Loading…",
      ready: "Synced",
      error: "Grant failed",
      save: "Save grants",
      currentGrantAt: "Current grant since {time}",
      notGrantedNow: "No active grant",
      historyTitle: "Authorization history (all domain agents)",
      historyEmpty:
        "No grant changes recorded yet. Saving scopes writes AGENT_DOMAIN_GRANT to audit JSONL.",
      historyTime: "Time",
      historyAgent: "Domain agent",
      historyScopes: "Scopes granted",
      historyAction: "Action",
      historyGranted: "Granted",
      historyRevoked: "Revoked",
      scopes: {
        "panel.browse": "Panel B · data browser",
        "panel.audit": "Panel D · audit",
        "panel.ledger": "Panel E · ledger",
        "panel.compliance": "Panel F · compliance",
        "tra.chat": "Chat / query tool",
        "tra.write": "TRA write tool",
      },
      registryTitle: "Logical agent registry (TRA)",
      registrySubtitle:
        "Metadata rows from domain_agents — a separate layer from the 13 agent packs above",
      registryUnavailable: "domain_agents table is not present in the current TRA database.",
      registryEmpty: "No logical agents match the current filters.",
      registrySummary: "{total} logical agents (partial {partial} · false {false})",
      registryImportBundled: "Import bundled registry (1000 rows)",
      registryImporting: "Importing domain agent registry…",
      registryImportDone: "Registry import complete — {total} logical agents.",
      registryImportError: "Registry import failed",
      registrySelectLabel: "Select logical agent",
      registrySelectPlaceholder: "— choose one —",
      registryFilterPack: "Pack",
      registryFilterEnabled: "enabled",
      registryFilterAll: "All",
      registryFilterPartial: "partial",
      registryFilterFalse: "false",
      registryColId: "ID",
      registryColName: "Name",
      registryColDomain: "Domain",
      registryColPack: "Pack",
      registryColEnabled: "Status",
      registryDetailId: "Agent ID",
      registryDetailName: "Name",
      registryDetailDomain: "Domain",
      registryDetailSubdomain: "Subdomain",
      registryDetailRegion: "Region",
      registryDetailInsurance: "Insurance",
      registryDetailPack: "Pack",
      registryDetailPackVersion: "Pack version",
      registryDetailScopes: "TRA scopes",
      registryDetailWrite: "TRA write",
      registryDetailRegistered: "Registered",
    },
    agentPackAuthoring: {
      title: "C2 · Agent Pack authoring",
      subtitle: "Validate and edit agent.pack.json via Phase 4 HTTP API",
      description:
        "Load a pack manifest, validate against the closed schema, then save to agentPacksDir when the gateway is configured for operator-owned packs.",
      loading: "Loading…",
      ready: "Ready",
      error: "Error",
      validating: "Validating…",
      saving: "Saving…",
      selectLabel: "Pack",
      selectPlaceholder: "— choose one —",
      editorLabel: "Manifest (JSON)",
      editorPlaceholder: '{ "id": "your-pack", ... }',
      loadBtn: "Load manifest",
      validateBtn: "Validate",
      saveBtn: "Save (PUT)",
      selectPackFirst: "Select a pack before loading.",
      invalidJson: "Editor contains invalid JSON.",
      missingPackId: "Manifest must include a non-empty id field before save.",
      loadFailed: "Failed to load pack detail.",
      loadDone: "Loaded pack {id}.",
      validateOk: "Manifest valid for pack {id}.",
      saveFailed: "Save failed.",
      saveDone: "Saved pack {id}.",
      writeDisabled:
        "Pack writes require gateway plugin config agentPacksDir (403 pack_write_disabled).",
      listReady: "{count} packs in registry.",
      newDraftBtn: "New draft",
      newDraftReady: "Loaded starter draft — edit id and fields, then Validate → Create.",
      createBtn: "Create (POST)",
      deleteBtn: "Delete",
      creating: "Creating…",
      deleting: "Deleting…",
      createDone: "Created pack {id}.",
      createFailed: "Create failed.",
      deleteDone: "Deleted pack {id}.",
      deleteFailed: "Delete failed.",
      deleteConfirm: "Delete agent pack {id}? This removes its directory under agentPacksDir.",
      defaultPackProtected: "Cannot delete the default agent pack.",
      packAlreadyExists: "Pack id already exists (409). Use Save (PUT) to update.",
      selectPackOrIdForDelete: "Select a pack or provide a manifest id before delete.",
    },
    agentBar: {
      label: "Domain agent",
      checking: "Checking grant…",
      granted: "Granted",
      notGranted: "Not granted",
      noPack: "No agent pack",
      notGrantedHint: "Grant this domain agent in Panel C before browsing data.",
    },
    browser: {
      title: "B · Trust runtime data browser",
      subtitle:
        "Read-only inspection of personal trust-runtime tables and Panel F subscribed reference/compliance data — with lineage",
      mountNote: "SQLite database status:",
      viewerLabel: "Data viewer",
      categoryLabel: "Dataset",
      categoryAll: "All tables",
      categoryPersonal: "Personal data",
      categorySubscribed: "Subscribed data",
      lineageTitle: "Data lineage",
      lineageEmpty: "Select a table to view upstream/downstream relationships and provenance.",
      lineageKindPersonal: "Personal",
      lineageKindSubscribed: "Subscribed",
      lineageKindReference: "Reference",
      lineageKindProvenance: "Provenance registry",
      lineageKindView: "Derived view",
      lineageUpstream: "Upstream",
      lineageDownstream: "Downstream",
      lineageLiveStandard: "Active compliance standard",
      lineageLiveReference: "NRDL reference sync",
      lineageLiveSources: "Registered sources in this table",
      lineageProvenanceFields: "Provenance columns",
      lineageFlowTitle: "Lineage flow",
      subscriptionsTitle: "Panel F subscription status",
      subscriptionsPharma: "Pharma compliance",
      subscriptionsNrdl: "NRDL reference",
      subscriptionsActive: "Active",
      subscriptionsInactive: "Not imported",
      subscriptionsSynced: "Synced",
      subscriptionsNotSynced: "Not synced",
      subscriptionsPharmaHint: "Import a compliance standard in Panel F to subscribe.",
      subscriptionsNrdlHint: "Sync NRDL reference rules in Panel F to subscribe.",
      subscriptionsDrugCount: "{count} drugs",
      subscriptionsRuleCount: "{count} rules",
      subscriptionsQuickJump: "Browse subscribed tables",
      provenanceLevel: "Level {level}",
      mounted: "Mounted",
      notMounted: "Not mounted",
      unknown: "Unknown",
      selectTable: "Select a table",
      loading: "Loading…",
      reload: "Refresh",
      noData: "No data",
      listError: "Error listing tables",
      loadError: "Error",
      subscriptionsLoadError: "Could not load subscription status",
    },
    chat: {
      title: "C · Trustworthy Q&A",
      placeholder: "Enter your question…",
      send: "Send",
      userLabel: "User",
      assistantLabel: "AI",
      noResponse: "(no response)",
    },
    audit: {
      title: "D · Runtime Audit",
      subtitle: "Mandatory pipeline stages — each chat run must record ≥5 JSONL events",
      sectionPipeline: "Chat pipeline gates (strict order)",
      pipelinePending: "No chat audit yet — run a GLP-1 question in Control UI TRA Console.",
      pipelineComplete: "All {total} pack stages SUCCESS for trail {trailId}.",
      pipelineIncomplete:
        "Incomplete trail ({count}/{total} pack stages) — DoD fail until all declared stages succeed.",
      pipelineBlocked: "Pipeline blocked at stage {step} — downstream stages must not run.",
      gateText2sql: "SQL summary only; read_only_verification required",
      gateQuery: "row_count + column names; no full row dump",
      gateRules: "Deterministic rule eval + active_ruleset id",
      gateDecision: "GLP-1 matrix + [Evidence #N] citations",
      gateLedger: "SHA-256 proof_hash linked to previous block",
      sectionCompliance: "Compliance & consent audit (JSONL)",
      sectionChat: "Latest chat pipeline",
      chatTrailLabel: "Audit trail ID:",
      compliancePlaceholder:
        "Loads DATA_CONSENT and COMPLIANCE_IMPORT events from state/tra-audit/events.jsonl.",
      complianceEmpty:
        "No compliance or consent audit events yet. Subscribe in Panel F or trigger data consent in Chat, then refresh.",
      complianceLoaded: "Loaded {count} compliance/consent audit event(s).",
      loading: "Loading audit events…",
      reload: "Refresh audit",
      loadError: "Failed to load audit",
      chatEmpty:
        "No chat pipeline audit yet. Ask a GLP-1 / health question in Chat (trustclaw_tra_query) or refresh after a run.",
      chatLoaded: "Loaded {count} step(s) from the latest chat audit trail (JSONL).",
      chatLoading: "Loading chat pipeline audit…",
      chatLive: "Live runtime context from the latest chat run.",
      chatCoordinator: "Agent pack «{pack}» (source: {source}).",
      chatMismatch:
        "Locked pack «{pack}» (source: {source}). OpenClaw agent suggests «{suggested}».",
      chatOtherAgent:
        "Live trail belongs to agent «{agent}» — switch the selector or chat with that agent.",
      apiUnavailable:
        "Audit API unavailable (404). Restart Gateway (`pnpm trustclaw:dev`) so /api/tra/audit/events is registered.",
      stepDataConsent: "Private data consent · DATA_CONSENT",
      stepAgentDomainGrant: "Domain agent grant · AGENT_DOMAIN_GRANT",
      stepComplianceImport: "External standard import · COMPLIANCE_IMPORT",
      stepReferenceSync: "NRDL reference sync · REFERENCE_SYNC",
      stepDeviceImport: "Device import · DEVICE_IMPORT",
      stepUser: "0 · User Input",
      stepText2sql: "1 · Text2SQL",
      stepQuery: "2 · Trust runtime query",
      stepRules: "3 · Rule Evaluation",
      stepDecision: "4 · GLP-1 Decision",
      stepLedger: "5 · Ledger Receipt",
      ledgerPending: "Awaiting ledger commit",
      evidenceTitle: "Evidence citations",
      evidenceTag: "[Evidence #{n}]",
      evidenceMissingValue: "missing",
    },
    ledger: {
      title: "E · Evidence Ledger",
      subtitle: "Append-only hash chain — verify links previous_evidence_hash → proof_hash",
      placeholder: "Unverified",
      receiptReady: "Chain verified",
      chainPending: "Awaiting verify",
      chainBroken: "Chain broken",
      blockHeight: "Ledger height",
      rootHash: "Root hash",
      proofLabel: "Proof JSON",
      copyProof: "Copy",
      copiedProof: "Copied",
      emptyNote:
        "No ledger receipts yet. Run a Chat turn (LEDGER_COMMIT audit), refresh Panel D, or wait for auto-sync.",
      loadError: "Ledger load failed",
    },
    compliance: {
      title: "F · Compliance Data Subscription",
      subtitle: "External standards & device data — consent-gated; each action → JSONL audit",
      description:
        "Choose a subscription type and delivery method to sync external pharma compliance standards or NRDL reference rules into local trust runtime. Consent is required; each action is audit-logged.",
      subscriptionTypeLabel: "Subscription type",
      subscriptionMethodLabel: "Delivery method",
      typePharmaCompliance: "Pharma compliance data",
      typeNrdlReference: "NRDL reference rules",
      typeDeviceData: "Third-party device data import",
      methodBundled: "Bundled package (one-click)",
      methodFile: "Local JSON file",
      methodUrl: "HTTPS remote URL",
      methodApi: "HTTPS API fetch",
      noStandard: "Not subscribed",
      activeStandard: "Active",
      sectionFile: "External standard file",
      sectionConsent: "User consent",
      fileLabel: "Select JSON package",
      fileHint: "Supports nrdl-ast-handshake v1 (e.g. GLP-1 analog standard pack).",
      consentLabel:
        "I consent to importing this external compliance standard for on-device medication checks",
      consentHint:
        "Import is blocked until consent is checked. Each import creates a consent session in the audit trail.",
      previewBtn: "Preview & validate",
      importBtn: "Import & activate",
      bundledBtn: "Import bundled GLP-1 v2",
      selectFile: "Select a JSON file first",
      previewFirst: "Preview and validate the package before importing",
      consentRequired: "User consent is required before import",
      previewing: "Validating…",
      importing: "Importing…",
      loadError: "Error",
      metaVersion: "Standard version",
      metaPublisher: "Publisher",
      metaPublisherSignature: "Publisher signature (metadata)",
      metaRelease: "Release date",
      metaSource: "Source file",
    },
    reference: {
      title: "NRDL reference subscription",
      description:
        "Sync read-only NRDL drug registry and payment rules into local trust runtime. Supports HTTPS URL or JSON file; personal data tables are never touched.",
      sectionUrl: "Remote subscription URL",
      urlLabel: "HTTPS package URL",
      urlHint:
        "Fetches JSON at sync time only (no runtime remote DB). localhost http allowed in dev.",
      sectionFile: "Local reference package",
      fileLabel: "Select JSON reference package",
      fileHint: "nrdl-reference v1 format (drugs + payment_rules).",
      sectionConsent: "Reference data consent",
      consentLabel:
        "I consent to syncing this external NRDL reference dataset for on-device rule evaluation",
      consentHint:
        "Separate from compliance AST import. Each sync is audit-logged as REFERENCE_SYNC.",
      sectionStatus: "Local reference status",
      statusEmpty: "No reference sync recorded",
      statusLoaded: "{drugs} drugs · {rules} rules · last sync {synced}",
      previewBtn: "Preview reference",
      syncBtn: "Sync to trust runtime",
      syncUrlBtn: "Sync from URL",
      bundledBtn: "Sync bundled GLP-1 reference",
      selectSource: "Enter a URL or select a JSON file",
      previewFirst: "Preview and validate before sync",
      consentRequired: "Reference sync requires consent",
      previewing: "Validating reference…",
      syncing: "Syncing reference…",
      loadError: "Error",
      metaVersion: "Package version",
      metaDrugs: "Drugs",
      metaRules: "Rules",
      metaSynced: "Last synced",
      metaSubscription: "Saved URL",
      unchanged: "Package unchanged — sync skipped",
    },
    device: {
      sectionApi: "Device API subscription",
      urlLabel: "HTTPS API URL",
      urlHint: "Fetch wearable/device export JSON. HTTPS only (localhost http allowed in dev).",
      hintLabel: "Device hint",
      hintPlaceholder: "e.g. Oura Ring, Apple Health export, glucometer API",
      sectionFile: "Local device JSON",
      fileLabel: "Choose device export JSON",
      fileHint: "Health/wearable JSON from vendor app or open platform.",
      consentLabel:
        "I consent to importing this external device data into local trust runtime for personal health analysis",
      consentHint:
        "Import uses Text2SQL-generated INSERT statements into SQLite; each import is DEVICE_IMPORT audit-logged.",
      previewBtn: "Preview Text2SQL",
      importBtn: "Import into trust runtime",
      selectSource: "Enter an API URL or choose a JSON file",
      previewFirst: "Preview INSERT SQL before import",
      consentRequired: "User consent is required before import",
      previewing: "Generating INSERT SQL…",
      importing: "Writing to trust runtime…",
      statusReady: "Ready",
      statusEmpty: "No device data imported yet",
      metaTables: "Target tables",
      metaStatements: "SQL statements",
    },
  },
};
