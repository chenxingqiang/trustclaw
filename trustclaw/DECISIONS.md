...
**平台 baseline 已落地（2026-07-05）。** 状态：`delivered` | `approved` | `deferred`。新架构或流程变更须新开决策项，不得静默偏离 `VISION.md` 五平面边界。

| ID  | 决策                                                                                | 状态      |
| --- | ----------------------------------------------------------------------------------- | --------- |
| D1  | TRA v1.1 schema（`trustclaw/tra/schema/v1.1.sql`）                                  | delivered |
| D2  | `extensions/trustclaw-tra` + `trustclaw/` 核心库；不改 OpenClaw core                | delivered |
...
| D7  | Text2SQL：LLM + SELECT 守卫（`trustclaw/tra/query.ts`）                             | delivered |
| D8  | 审计：`state/tra-audit/` JSONL                                                      | delivered |
| D9  | 账本：`state/tra-evidence/` 哈希链                                                  | delivered |
