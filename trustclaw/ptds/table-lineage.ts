...
function readScalar<T>(
  db: ReturnType<typeof openPtdsDatabase>,
  sql: string,
  params: unknown[] = [],
): T | undefined {
...
function readActiveCompliance(db: ReturnType<typeof openPtdsDatabase>):
  | {
      standard_id: string;
      ruleset_hash: string;
...
function readReferenceSyncState(db: ReturnType<typeof openPtdsDatabase>):
  | {
      version_id: string;
      package_hash: string;
      subscription_url: string | null;
...
export function buildTableLineage(
  ...
  const kind = classifyPtdsTable(table);
  ...
  const dbPath = resolvePtdsDbPath(
