...
export type TraSubscriptionType = "pharma-compliance" | "nrdl-reference" | "device-data";
...
export const TRA_TABLE_CATALOG: Record<string, TableCatalogEntry> = {
  user_profile: {
    kind: "personal",
...
export function getTableCatalogEntry(table: string): TableCatalogEntry | undefined {
  return TRA_TABLE_CATALOG[table];
}

export function classifyTraTable(table: string): TraTableKind {
  return TRA_TABLE_CATALOG[table]?.kind ?? "personal";
}

export function isSubscribedTable(table: string): boolean {
  const entry = TRA_TABLE_CATALOG[table];
  return entry?.kind === "subscribed" || entry?.subscription_type !== undefined;
}

/** Default Panel B tables (D12 + subscribed reference/compliance). */
export const TRA_BROWSER_DEFAULT_TABLES = [
  "body_anthropometrics",
  "lab_test_results",
  "clinical_diagnoses",
  "medication_compliance_standards",
