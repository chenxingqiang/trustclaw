...
import { bootstrapTraDatabase, TRA_LOCAL_USER_ID } from "./db.js";
...
import { TRA_INIT_DEFAULTS, type TraInitRequest, type TraInitResult } from "./types.js";

function clearTraPersonalData(db: DatabaseSync): void {
  ...
  db.prepare("DELETE FROM prescription_context WHERE user_id = ?").run(TRA_LOCAL_USER_ID);
  db.prepare("DELETE FROM user_profile WHERE user_id = ?").run(TRA_LOCAL_USER_ID);
}
...
export function applyTraInitRequest(db: DatabaseSync, request: TraInitRequest): number {
  clearTraPersonalData(db);

  const name = request.patientName?.trim() || TRA_INIT_DEFAULTS.patientName;
  ...
  db.prepare(
    `INSERT INTO user_profile (user_id, name, birth_date, biological_sex)
     VALUES (?, ?, ?, ?)`,
  ).run(TRA_LOCAL_USER_ID, name, birthDateFromAge(request.age), biologicalSex);
  inserted += 1;
  ...
  ).run(now, heightM, request.weight, TRA_LOCAL_USER_ID);
  inserted += 1;
...
export function initializeTra(
  request: TraInitRequest,
  ...
  try {
    ...
    const recordsInserted = applyTraInitRequest(db, request);
    db.close();
    return {
      status: "success",
