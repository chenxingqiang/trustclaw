...
export async function previewDeviceImport(
  ...
  options: { llm: DeviceImportLlm; dbPathOrOverrides?: string | TraPathOverrides },
  ...
  try {
    if (!resolvePrimaryUserId(db)) {
      return {
        status: "error",
        message:
          "Trust runtime is not initialized. Complete Panel A init before importing device data.",
      };
    }
  } finally {
    db.close();
...
export async function importDeviceData(
  ...
  try {
    ...
    try {
      ...
      return {
        status: "success",
        message: "Third-party device data imported into TRA.",
