...
describe("trustclaw/tra browse subscriptions", () => {
  const seedCompliance = JSON.parse(readFileSync(TRA_SEED_GLP1_AST_V2_JSON, "utf8"));
  const seedReference = JSON.parse(readFileSync(TRA_SEED_NRDL_REFERENCE_GLP1_JSON, "utf8"));
  ...
  it("reports inactive pharma and unsynced NRDL before Panel F imports", () => {
    ...
    try {
      initializeTra(TRA_INIT_DEFAULTS, dbPath);
