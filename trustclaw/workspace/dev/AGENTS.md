...
- Panel **A**: `POST /api/ptds/init` — mount personal data into the trust runtime
- Panel **B**: browse local SQLite tables
- Panel **C**: chat — use **`trustclaw_ptds_query`** for pack / eligibility questions
...
3. Never invent vitals or rule outcomes; use **`trustclaw_ptds_query`** when the trust runtime is mounted.
