...
- **术语**：产品称 **TRA**；遗留 HTTP/DB 前缀 `ptds`（`/api/ptds/*`、`local_ptds.db`）见 `DECISIONS.md` D25
...
| 目标                 | 标准                                                                       | 不达标表现                                               |
| -------------------- | -------------------------------------------------------------------------- | -------------------------------------------------------- |
| **不出域**           | TRA SQLite（`local_ptds.db`）、审计 JSONL、consent 文件均在本地 `state/`；无静默外发         | 模型 prompt 含未授权原始行、或未走 tool 直接猜 vitals    |
...
| -------------------------- | ----------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | ---------------------------------------------------------- | --------------------------------------------- |
| **A · 内部 TRA 个人数据** | `user_profile`、`body_anthropometrics`、`lab_test_results`、`clinical_diagnoses`、`medication_history` 等 v1.1 表 | `state/local_ptds.db`                                                | Gateway `requireApproval`（`before_tool_call`）            | `DATA_CONSENT` / `PTDS.Consent`               |
...
| TRA mounted      | init / tool / chat                        | 未 `POST /api/ptds/init`                              |
...
| **产品契约优先**   | 数据走 TRA schema（`trustclaw/ptds/`）+ 冻结 API；规则走 SQLite/导入标准；**禁止** TS 硬编码垂直规则（D14） |
...
| ----------------- | ------------------------------ | ----------------------------------------------------------- | ---------------------------------------------- |
| **Data**          | `src/state/` + Kysely/SQLite   | TRA schema、init、SELECT 守卫、参考同步、血缘              | `trustclaw/ptds/`                              |
...
| `ptds_reset`          | `POST /api/ptds/reset`                               | Operator    | 清空个人 TRA 数据行 + audit/ledger                     |
...
- **术语**：产品逻辑用「本地用户 / 个人 TRA 数据 / 种子规则」；避免 `demo_*` 字段名进入 API 或持久化层。遗留 API/路径前缀 `ptds` 见 D25。
...
| -------------------- | ---------------------------------------------------------------------- | ------ | ------------------------------------- |
| TRA store            | `trustclaw/ptds/`                                                      | ✓      | kysely-sync pattern                   |
...
| TRA Agent 工作台    | `ui/src/ui/views/trustclaw-ptds-workbench.ts`                          | ✓      | Control UI Chat + iframe 侧栏         |
...
| **TRA Agent 工作台**    | `http://127.0.0.1:19001/` → Control UI **TRA Console** 标签                                          | **Agent 实例交互**：中心原生 Chat（`trustclaw_ptds_query` 等工具）、Pack 人设；左右 **iframe** 嵌入 Runtime Console 的 `?embed=left` / `?embed=right` 侧栏。 | 中心 Chat；侧栏复用 A/C/B 与 D/E/F                                                                               |
...
- Local TRA SQLite (`state/local_ptds.db`)
